...
# Source to the ints, since this is used for colors
SOURCE_TO_INT = {
    ScoreSource.CACHE: 1,
...
def df_to_matrix(df):
    # Build the matrix for visualization
...
def visualize_grid(df, output_file):
    # Display the gird of done tasks
...
def main():
    ...
    df_e_p = get_benchmark_dataframe(get_models).dropna()
    df_m_p = get_benchmark_dataframe(
        get_multilingual_models, multilingual=True
    ).dropna()
    visualize_grid(df_e_p, "model_dataset_filtered.png")
    save_stats(df_e_p, "model_dataset_filtered_stats.json")
    visualize_grid(df_m_p, "model_multilingual_dataset_filtered.png")
    save_stats(df_m_p, "model_multilingual_dataset_filtered_stats.json")
